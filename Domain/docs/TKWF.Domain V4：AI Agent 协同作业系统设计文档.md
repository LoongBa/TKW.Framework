...
### **L3: Business Implementation Delta（业务增量库）**

- **性质**：**增量记录 （Incremental Log）**。

- **内容**：
  
  - 手写`Controller`的复杂业务逻辑描述。
  
  - `Agent`自己总结的“避坑指南”或特定业务补丁说明。
...
| **执行** | 编写代码 / 修改 Entity       | OMO      | 接收到指令       |
| **同步** | **调用`cli agent`更新 L2** | OMO      | 代码修改完成且编译通过 |
| **反思** | **在 L3 记录本次修改的增量逻辑**   | OMO      | 提交任务前       |
