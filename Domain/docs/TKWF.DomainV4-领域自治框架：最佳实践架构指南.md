...
1. **开放情况**：建议不要直接开放`DomainService/DataService`，而是通过控制器包装，以便自动触发`AOP约束`；

2. **定义契约**：手写`{Entity}Controller` 接口`I{Entity}Controller`，并继承 `IAopContract`；

3. **标注拦截器**：在接口或方法上标注 `[AuthorityFilter]`、`[Transactional]` 等`契约特性`；
   ——建议在控制器的方法上标注`契约特性`，随后用 Cli 自动提取生成`契约接口`（自动复制特性）

4. **调用逻辑**：通过 `User.Use<PaymentLogService>()` 或`User.UseAop<PaymentLogController>()`调用服务和控制器。
