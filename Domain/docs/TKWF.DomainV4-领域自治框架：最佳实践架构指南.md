...
**领域自治**的核心目标是**让开发者**从“写代码满足框架”变为“**写代码只关注业务**”。
通过物理级的执行流隔离，**`TKWF.Domain` 领域自治框架**带来了以下直观收益：
...
V4 架构将原本需要手动“维系”的环境稳定性（DI 注册、连接回收、环境绑定、逻辑解耦），通过 **SG 静态装配** 和 **自动化执行流调度** 变成了底层基础设施，让开发者能像写本地工具类一样编写分布式业务系统。
...
| **层级**     | **组件名称**        | **核心职责**             | **可见性（Visibility）**                        | **基类与特性**              | 存储位置           |
| ---------- | --------------- | -------------------- | ------------------------------------------ | ---------------------- | -------------- |
| **元数据层**   | **Entity/View** | 定义数据模型、`DB 映射`与验证契约。 | **Public**（支持 GraphQL 穿透与 Dto 投影）          | `IDomainEntity`        | /Entities      |
| **数据服务层**  | **Dto**         | 用于外部访问，自动转换。         | **Public**（跨程序集契约）                         | `IDomainDto`           | /Entities/Dtos |
|            | **DataService** | 内部`CRUDQ`访问。         | **Internal**（默认封闭，通过 `IsPublicService` 开启） | `DomainDataService`    | /DataServices  |
| **业务控制器层** | **Controller**  | 业务编排、`AOP`拦截控制。      | **Internal**（实现封闭） / **Public**（接口开放）      | `DomainControllerBase` | /Controllers   |

> **核心原则**：
> 
> 领域框架的源生成器会自动发现 `DomainDataServiceBase/DomainReadOnlyDataServiceBase`、`DomainServiceBase`、`DomainControllerBase` 的子类；
> 
> 随后生成 `RegisterGeneratedDomainServices()`自动注册方法，由框架初始化器内部自动调用。
> 
> 注册和调用时会自动判断、验证限制规则：
> 
> * `Controller`：继承自`DomainControllerBase`，`SG` 自动生成`契约接口`和`代理类`，并自动注册、启用 `AOP 拦截模式`；
>   ——必须通过`User.UseAop()`方式，指定`契约接口`来使用；
> 
> * 基于`Entity`模板生成的`DataService`，则直接用类本身 `AsSelf` 注册，通过`User.Use()`方式，指定`类`来使用。
...
### 第二步：领域服务与数据服务（DomainService/DataService）

> 数据服务是一种特殊的领域服务，基于注入的`IEntityDAC`实现数据库操作。
> 生成的`{Entity}DataService`自动继承`DomainDataServiceBase`（`IsView == true`，视图继承自`DomainReadOnlyDataServiceBase`）

- **开放情况**：表现层可以直接访问公开的业务方法（已有角色权限判断机制），但推荐关闭访问，仅开放`Controller`统一访问入口；

- **编写方式**：数据服务由`xCodeGen Cli`根据`Entity`自动生成；`DomainService`调用`Snippet`模板生成空文件，然后编写业务代码；

- **不参与 AOP**：它们属于原子操作，不参与 AOP——不提取接口，不生成`AOP代理类（装饰器）`；

- **注册方式**：由领域框架自动注册到领域容器。直接以原始类注册，不依赖接口。

### 第三步：业务控制器（DomainController）

开发者根据业务需求创建 `{Entity}Controller.cs`作为对外的统一访问接口：

1. **开放情况**：建议不要直接开放`DomainService/DomainDataService`，而是通过控制器包装，以便自动触发`AOP约束`；

2. **定义契约**：手写`{Entity}Controller`继承 `DomainControllerBase`，`SG`会根据控制器方法自动提取生成 **`public` 契约接口**；

3. **静态代理**：`SG`生成一个 **`internal` 装饰器类** 。该装饰器实现了**契约接口**，并持有`internal`**控制器**实例，通过底层 DI 容器完成装配 ；
   ——外部通过**契约接口**访问控制器，所以`internal`的装饰器、控制器并不影响访问，但可以更好地**保护边界**。

4. **标注拦截器**：在接口或方法上标注 `[AuthorityFilter]`、`[Transactional]` 等`契约特性`；
   ——建议在控制器的方法上标注`契约特性`，随后用`Cli`自动提取生成`契约接口`（自动复制特性）

5. **调用逻辑**：通过 `User.Use<PaymentLogService>()` 或`User.UseAop<PaymentLogController>()`调用服务和控制器。
...
### 4. 进阶：AOP 拦截契约与领域上下文透传

在 V4 架构中，AOP 不再是黑盒。
由于采用了静态装饰器模式，拦截器在执行业务逻辑前，会将当前的执行细节“压入”异步执行流中。
开发者可以利用`DomainUser`这一核心入口，在任何层级（`Controller`、`Service` 甚至实体内部）获取这些元数据。

#### A. 上下文获取机制

框架通过 `AsyncLocal` 实现了作用域内的上下文隔离。开发者无需传递复杂的参数，直接通过 `User` 即可访问：

- **`User.CurrentInvocation`**：获取当前拦截的方法名、参数数组、目标实例等原始信息。

- **`User.GetCurrentMethodMeta()`**：**（推荐）** 基于`SG`预生成的 `ProjectMetaContext`，高性能、零反射地获取方法的 XML 注释（Summary）、特性列表（Attributes）等静态元数据。

在任何继承自 `DomainServiceBase` 或 `DomainControllerBase` 的类中，无论是否加入拦截特性，都可以直接使用：

```csharp
public async Task ProcessOrderAsync(long orderId)
{
    // 1. 获取运行时信息 (方法名、传入参数)
    var invocation = User.CurrentInvocation;
    if (invocation != null)
    {
        var targetMethod = invocation.MethodName; // "ProcessOrderAsync"
        var firstArg = invocation.Arguments[0]; // orderId 的值
    }

    // 2. 获取 SG 预生成的静态元数据 (Summary, 特性)
    var meta = User.GetCurrentMethodMeta(); 
    var summary = meta?.Summary; // 自动获取 XML 注释中的内容
}
```

#### B. 典型应用场景

1. **动态审计与溯源**：在底层 `DataService` 中，通过 `User.CurrentInvocation.MethodName` 识别当前操作是源自“后台管理导出”还是“移动端快速下单”。

2. **细粒度数据权限**：在过滤器中拦截参数，结合 `User.CurrentInvocation.Arguments` 中的具体 ID，判断用户是否有权操作该特定行数据。

3. **无反射元数据读取**：在通用逻辑中读取方法标注的自定义特性（如 `[CacheKey(Prefix="User")]`），实现动态业务配置。

#### C. 业务控制器完整示例 (AOP + Context)

以下示例展示了如何利用 **Snippet** 快速创建控制器，并配合`AOP 上下文`实现业务逻辑：

```csharp
// UserController.cs (手写的部分类，可通过 snippet 创建)
namespace DMP_Lite.Domain.Controllers;

[LoggingFilter] // 控制器级拦截：自动记录所有方法的出入参日志
internal partial class UserController(DomainUser<DmpUserInfo> user)     : DomainControllerBase<DmpUserInfo>(user)
{
    /* 1. 开发者输入 'tkwd-controller' 快捷创建此类       2. SG 自动提取 Login 及其特性生成 IUserController 接口       3. SG 生成 UserControllerDecorator 进行静态 AOP 转发    */

    [AllowAnonymousFlag] // 契约特性：标记此方法允许匿名访问
    [Transactional]      // 契约特性：自动开启事务作用域
    public async Task<DmpUserInfo> Login(string userName, string passwdHashed)
    {
        // 场景展示：在方法内部感知 AOP 环境
        var meta = User.GetCurrentMethodMeta(); 
        Console.WriteLine($"执行方法：{meta?.Name}，说明：{meta?.Summary}");

        // 业务逻辑编排
        var userInfo = await Use<UserAuthService>().VerifyAsync(userName, passwdHashed);

        // 记录登录足迹
        await Use<UserLogDataService>().CreateAsync(new UserLogDto {
            Action = User.CurrentInvocation?.MethodName, // 自动获取 "Login"
...
- **接口绑定**：`Controller` 必须继承 `DomainControllerBase`，以便`SG`自动提取契约接口、生成装饰器类。
  并且在控制器类和方法上添加所需的AOP拦截器特性，以便拦截器根据特性运行对应的逻辑，如权限控制、日志审计。

- **零反射保障**：优先使用 `User.GetCurrentMethodMeta()`。
  由于`SG`随项目生成了`IProjectMetaContext`的实现代码，它在运行时直接从内存字典中检索元数据，性能远高于标准的`MethodInfo`反射。

- **透传范围**：即使在不被`AOP`直接拦截的 `DataService` 内部，只要它处于由 `Controller` 发起的异步调用链中，上下文依然可以通过 `User` 访问。

---

## 五、服务注册与解析的架构守卫

在 V4 静态`AOP`体系下，框架通过编译期限制和运行期断言，强制执行“**契约优先**”原则。

### 1. 注册守卫 (Registration Guards)

在 `DomainHost` 初始化阶段，系统会对注册路径进行严格分流：

- **`AddAopService` (强制用于控制器)**：
  
  - **职责**：同时注册原始实现类（`AsSelf`）与契约接口（指向装饰器工厂）。
  
  - **校验**：确保实现类与装饰器均完整实现了契约接口。

- **`AddService` (严禁用于控制器)**：
  
  - **职责**：用于注册无需 `AOP` 拦截的原子服务（如 `DataService`）。
  
  - **守卫**：若尝试将实现了 `IAopContract` 的类（控制器）通过此方法注册，框架将抛出异常，强制要求使用 `AddAopService`。

### 2. 解析守卫 (Resolution Guards)

在业务代码中，通过 `DomainUser` 访问服务时需遵循以下规约：

- **`User.UseAop<TContract>()`**：
  
  - **约束**：泛型参数 `TContract` **必须为接口**。
  
  - **守卫**：严禁传入具体的控制器类名。若传入类名，解析将抛出异常，以防止开发者绕过拦截器直接获得原始实例。

- **`User.Use<TService>()`**：
  
  - **职责**：获取零反射开销的原子服务实例。
  
  - **守卫**：在 `DEBUG` 模式下，若解析目标属于 `IAopContract`（控制器实现类），框架将触发断言报错，引导开发者改用 `UseAop<接口>()`。

### 3. 可见性规约 (Access Modifiers)

- **物理隔离**：推荐将 `Controller` 和 `DataService` 声明为 `internal` 。
  
  - **控制器**：其 AOP 装饰器会自动跟随控制器的访问级别（`internal`），确保外部项目只能通过 `public` 接口访问，强制触发拦截逻辑 。
  
  - **数据服务**：默认 `internal` 阻止表现层绕过业务层直接操作数据库 。若确需公开（如跨库共享原子服务），可通过 `[DomainGenerateCode(IsPublicService = true)]` 显式开启 。

- **GraphQL 穿透支持**：
  
  - **`Entity` 始终保持 `public`** 。这允许`GraphQL`等中间件直接读取实体结构进行字段投影（`Projections`），在不暴露修改逻辑的前提下，实现极速的查询性能 。

> **架构提示**：这一套“物理隔离 + 注册拦截 + 运行断言”的组合规则，确保了领域层在任何环境下（Web、测试、控制台）都能维持`AOP 约束`的绝对稳定性。
...
这对于系统管理后台和`AI Agent`的“**技能发现**”至关重要。一行代码获取列表：

```csharp
// 从单例 Host 中获取当前所有已注册的领域服务清单
var registrations = DomainHost<TUserInfo>.Root.Registrations;

foreach (var reg in registrations)
{
    Console.WriteLine($"接口: {reg.ServiceInterface?.Name}");
    Console.WriteLine($"实现: {reg.Implementation.Name}");
    Console.WriteLine($"类型: {reg.Type}"); // Entity, Controller, Service 等
    Console.WriteLine($"AOP 代理: {reg.ProxyType?.Name ?? "无"}");
    Console.WriteLine("-------------------------");
}
```
...
- **健康检查**：在系统启动后的 `Diagnostic` 页面，列出所有服务，并标记哪些是经过 AOP 装饰的，哪些是原始注册。

- **自动化 API 文档**：不再需要 `Swagger` 扫描，直接遍历这个列表就能生成一份完美的“领域能力地图”。

- **AI Agency MD**：可以将这个列表喂给 AI 助手，它能立刻理解当前系统具备哪些“动作`Actions`”可以被调用。
...
xCodeGen 提供初始化脚手架功能、代码模板（领域服务类、控制器类）

xCodeGen 自动提取 AOP 契约

xCodeGen 可生成 Agency MD 知识地图
