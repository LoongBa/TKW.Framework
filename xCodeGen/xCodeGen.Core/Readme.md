...
- Ver 1.2：同步最新代码（RazorLight 引擎确认、子项目分层完善）；新增对 Entities 目录生成 Repository & DTO 的端到端示例；补充一步步配置指南，用于检验与完善生成器最佳实践。

此文档作为 TKW.Framework 的工具链补充，与 TKWF.Domain 领域自治思路对齐：自动化生成领域层代码（如 DTO、Repository），减少样板，确保领域模型（Entities）作为单一事实来源。后续迭代将记录在 xCodeGen 仓库根 README.md 中，便于持续讨论。
...
## 1. 详细示例：从 Entities 目录生成 Repository & DTO

以下是针对 TKWF.Domain 项目中 Entities 目录下文件的端到端示例。假设项目结构为：
...
- 构建项目，确认生成前置。
- **检验点**：测量性能（<1s for small Entities）。如果慢，优化 RazorLight 缓存（TemplateExecutor.UseCaching = true）。
