    ...
    public partial class CodeMetaDataExtractor : IMetaDataExtractor, IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(combinedProvider, (spc, combined) =>
            {
                ...
                meta = ExtractFrameworkInterfaceMeta(spc, compilation, "TKW.Framework.Domain.Interfaces.IEntityReadOnlyDAC`1", "IEntityReadOnlyDAC");
                allMetadatas.Add(meta);
#endif
        ...
        private ClassMetadata ExtractFrameworkInterfaceMeta(SourceProductionContext context, Compilation compilation, string fullyQualifiedName, string outputClassName)
        {
            // 1. 获取接口符号
            var symbol = compilation.GetTypeByMetadataName(fullyQualifiedName);
            if (symbol == null) return null; // 找不到时返回 null
            ...
            var metadata = new ClassMetadata
            {
                ...
                GenerateCodeSettings = new Dictionary<string, object>() // 初始化配置字典
            };
            ...
            // 4. 调用生成逻辑 (生成 IEntityDACMeta.g.cs)
