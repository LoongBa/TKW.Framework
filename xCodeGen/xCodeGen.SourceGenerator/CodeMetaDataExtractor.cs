...
namespace xCodeGen.SourceGenerator
{
    ...
    public partial class CodeMetaDataExtractor : IMetaDataExtractor, IIncrementalGenerator
    {
        ...
        public static readonly SymbolDisplayFormat ShortTypeFormat = SymbolDisplayFormat.MinimallyQualifiedFormat;
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            // 2. 获取项目配置信息
            var projectInfoProvider = context.CompilationProvider.Combine(context.AnalyzerConfigOptionsProvider).Select((
                pair, _) => new ProjectInfo(pair.Right, pair.Left));
            // 管道：( (List<Metadata>, Compilation), ProjectInfo )
            var combinedProvider = candidateTypes.Collect()
                .Combine(context.CompilationProvider)
                .Combine(projectInfoProvider);
            // 3. 注册最终输出
            context.RegisterSourceOutput(combinedProvider, (spc, combined) =>
            {
                var ((infos, compilation), projectInfo) = combined;
                // 提取框架公共接口（作为 DomainMap 的公共内容，节省 Token）
                ExtractFrameworkInterfaceMeta(spc, compilation, "TKW.Framework.Domain.Interfaces.IEntityDAC`1", "IEntityDAC");
                ExtractFrameworkInterfaceMeta(spc, compilation, "TKW.Framework.Domain.Interfaces.IEntityReadOnlyDAC`1", "IEntityReadOnlyDAC");
        ...
        {
            // 找出所有的 DAC 相关接口（同时查 IEntityDAC 和 IEntityReadOnlyDAC）
            var dacInterfaces = classSymbol.AllInterfaces.Where(i =>
                i.Name == "IEntityDAC" || i.Name == "IEntityReadOnlyDAC");

            foreach (var dacInterface in dacInterfaces)
            {
        ...
        private void ExtractFrameworkInterfaceMeta(SourceProductionContext context, Compilation compilation, string fullyQualifiedName, string outputClassName)
        {
            // 1. 获取接口符号
            ...
            {
                Namespace = symbol.ContainingNamespace?.ToDisplayString() ?? "Unknown",
                ClassName = outputClassName,
                FullName = symbol.ToDisplayString(),
                Type = MetaType.Interface,
                Summary = $"领域数据访问标准契约 ({outputClassName})",
                BaseType = "object",
                SubDomain = DefaultSubDomain,
                SourceHash = "", // 避免 EscapeString 报错
                // 关键：必须初始化这些集合，否则 BuildMetaCode 中的 foreach 会报 NRE
                Attributes = new System.Collections.ObjectModel.Collection<AttributeMetadata>(),
                Properties = new System.Collections.ObjectModel.Collection<PropertyMetadata>(),
                Methods = new System.Collections.ObjectModel.Collection<MethodMetadata>()
            };
            ...
            foreach (var m in symbol.GetMembers().OfType<IMethodSymbol>())
            {
                ...
                {
                    Name = m.Name,
                    ReturnType = m.ReturnType.ToDisplayString(ShortTypeFormat),
                    IsAsync = m.IsAsync,
                    Origin = MethodOrigin.Infrastructure,
                    Summary = "标准数据操作方法",
                    AccessModifier = "public",
                    SourceFile = "Framework.Internal", // 标记来源
                    Parameters = m.Parameters.Select(p => new ParameterMetadata
                    {
                        Name = p.Name,
                        TypeName = p.Type.ToDisplayString(ShortTypeFormat),
                        Summary = ""
                    }).ToList(),
                    Attributes = new List<AttributeMetadata>() // 初始化方法的特性集合
                });
            }

            // 4. 调用现有的生成逻辑
            GenerateMetaFile(context, metadata);
        }
        private void EnrichAndHashMetadatas(ref List<ClassMetadata> allMetadatas)
        {
            ...
            foreach (var m in allMetadatas)
            {
                ...
                if (genCodeAttr != null)
                {
                    ...
                    // --- SubDomain 提取逻辑 (特性优先，命名空间兜底) ---
                    var subDomain = GetStringProp(genCodeAttr, "SubDomain");
                    if (string.IsNullOrWhiteSpace(subDomain))
                    {
                        // 按照约定，解析命名空间。例如: DMP_Lite.Domain.Entities.Sales -> 提取 "Sales"
                        var nsParts = m.Namespace.Split('.');
                        var targetIndex = Array.FindIndex(nsParts, p =>
                            p.Equals("Entities", StringComparison.OrdinalIgnoreCase) ||
                            p.Equals("DataServices", StringComparison.OrdinalIgnoreCase));

                        if (targetIndex >= 0 && targetIndex < nsParts.Length - 1)
                        {
                            subDomain = nsParts[targetIndex + 1];
                        }
                    }
                    m.SubDomain = string.IsNullOrWhiteSpace(subDomain) ? DefaultSubDomain : subDomain;
                    // ---------------------------------------------------
                    filteredList.Add(m);
                    continue;
                }
            ...
            foreach (var entity in entities)
            {
                ...
                if (entity.Service != null)
                {
                    ...
                    entity.Service.SubDomain = entity.SubDomain;
                }
                if (entity.Controller != null)
                {
                    entity.Controller.UserType = entity.UserType;
                    entity.Controller.SubDomain = entity.SubDomain;
                }
