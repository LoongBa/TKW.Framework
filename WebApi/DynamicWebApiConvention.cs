...
{
    public class DynamicWebApiConvention(ISelectController selectController, IActionRouteFactory actionRouteFactory)
        : IApplicationModelConvention
    {
        public void Apply(ApplicationModel application)
        {
            foreach (var controller in application.Controllers)
            {
                ...
                if (!(selectController is DefaultSelectController) && selectController.IsController(type))
                {
                    controller.ControllerName = controller.ControllerName.RemovePostFix(AppConstants.ControllerPostfixes.ToArray());
        ...
        private void ConfigureArea(ControllerModel controller, DynamicWebApiAttribute attr)
        {
            if (!controller.RouteValues.ContainsKey("area"))
            {
                ...
                else if (!string.IsNullOrEmpty(AppConstants.DefaultAreaName))
                {
                    controller.RouteValues["area"] = AppConstants.DefaultAreaName;
                }
            }
        ...
        {
            if (AppConstants.FormBodyBindingIgnoredTypes.Any(t => t.IsAssignableFrom(parameter.ParameterInfo.ParameterType)))
            {
                return false;
            }
        ...
        private static string GetRestFulActionName(string actionName)
        {
            ...
            var appConstantsActionName = AppConstants.GetRestFulActionName?.Invoke(actionName);
            if (appConstantsActionName != null)
            {
                return appConstantsActionName;
            }

            // default process action name.

            // Remove Postfix
            actionName = actionName.RemovePostFix(AppConstants.ActionPostfixes.ToArray());

            // Remove Prefix
            var verbKey = actionName.GetPascalOrCamelCaseFirstWord().ToLower();
            if (AppConstants.HttpVerbs.ContainsKey(verbKey))
            {
        ...
        private static string GetHttpVerb(ActionModel action)
        {
            ...
            var verb = AppConstants.HttpVerbs.TryGetValue(verbKey, out var httpVerb) ? httpVerb : AppConstants.DefaultHttpVerb;
            return verb;
        }

        private AttributeRouteModel CreateActionRouteModel(string areaName, string controllerName, ActionModel action)
        {
            var route =  actionRouteFactory.CreateActionRouteModel(areaName, controllerName, action);
